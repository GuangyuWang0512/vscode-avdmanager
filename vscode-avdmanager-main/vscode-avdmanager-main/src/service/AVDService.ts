...
export class AVDService extends Service {
    ...
    async createAVD(avdname: string, path: string, imgname: string, device: number = -1,devicename:string) {
        const avdHome = this.manager.getConfig().avdHome;
        let name=devicename.toLowerCase().replace(/ /g, "_");
        if(name==="pixel_xl"){
            name="pixel_xl_silver";
        }
        if(name==="small_phone"){
            name="pixel_silver";
        }
        if(name==="medium_phone"){
            name="pixel_7a";
        }
