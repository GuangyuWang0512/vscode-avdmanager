...
export class AVDTreeView {
    ...
    async createAVDDiag(path: string, name: string) {
        ...
        const deviceName=(device as AVDDeviceQuickPickItem)?.avdDevice?.name ?? "";
        if (deviceId === -2) {
            showMsg(MsgType.info, "Please Select one of the AVD device definition.");
            return;
        }

        await this.manager.avd.createAVD(newAvdName, path, name, deviceId,deviceName);
        const sdkHome=process.env.ANDROID_SDK_HOME||"";
        const command=`echo hw.keyboard=yes >> "${sdkHome}\\.android\\avd\\${newAvdName}.avd\\config.ini"`;
